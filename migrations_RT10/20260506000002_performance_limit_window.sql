-- RT1.0 — Item 1.3: LIMIT + window functions na get_performance_metrics
-- Problema: CROSS JOIN health_stats escala quadraticamente com 300 lojas
--           efficiency_agg sem LIMIT retorna todas as combinações
-- Solução:  window functions MAX() OVER () eliminam o CROSS JOIN
--           LIMIT 50 no health, LIMIT 30 no efficiency
-- Data: 2026-05-06
-- Inclui: fix do cast pure_date (1.1) já aplicado
...
BEGIN
  ...
  -- LIMIT 30: evita retornar todas as combinações aparelho x linha
  efficiency_agg AS (
  ...
  -- Window functions substituem o CROSS JOIN: sem produto cartesiano
  health_agg AS (
    SELECT
      hb.store_name,
      hb.total_sessions,
      hb.qtd_aparelhos,
      hb.qtd_dias,
      ROUND((hb.total_sessions::numeric / NULLIF(MAX(hb.total_sessions) OVER (), 0) * 40)::numeric, 1) AS volume_score,
      ROUND((hb.qtd_aparelhos::numeric / NULLIF(MAX(hb.qtd_aparelhos) OVER (), 0) * 30)::numeric, 1) AS diversity_score,
      ROUND(
        CASE
      ...
      LEAST(100, GREATEST(0,
        ROUND((
          (hb.total_sessions::numeric / NULLIF(MAX(hb.total_sessions) OVER (), 0) * 40) +
          (hb.qtd_aparelhos::numeric / NULLIF(MAX(hb.qtd_aparelhos) OVER (), 0) * 30) +
          CASE
            WHEN hc.stddev_daily_sessions IS NOT NULL AND hc.avg_daily_sessions > 0 THEN
              GREATEST(0, 30 - ((hc.stddev_daily_sessions / hc.avg_daily_sessions) * 30))
            WHEN hb.qtd_dias >= 3 THEN 25
